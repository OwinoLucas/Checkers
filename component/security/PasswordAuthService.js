...
const Base = require('evado/component/security/PasswordAuthService');
...
module.exports = class PasswordAuthService extends Base {
    ...
    getUserByPhone (phone) {
        const query = this.spawnUser().findByPhone(phone);
        return query.one();
    }

    async login (phone, password, user) {
        const identity = await this.getUserByPhone(phone);
        if (!identity) {
